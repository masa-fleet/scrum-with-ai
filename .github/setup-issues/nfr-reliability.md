...
- [ ] 可用性目標（SLA/SLO 例: 99.9% = 月 43 分以内のダウンタイム）が定義されている
- [ ] RTO（Recovery Time Objective、例: 障害発生から 1 時間以内に復旧）が定義されている
- [ ] RPO（Recovery Point Objective、例: 最大 15 分分のデータ損失まで許容）が定義されている
- [ ] バックアップ方針（頻度・保持期間・世代数）が定義されている
- [ ] 障害検知・アラート方針（モニタリング対象・通知先）が決定されている
- [ ] フェイルオーバー戦略（アクティブ-スタンバイ / アクティブ-アクティブ、リージョン冗長等）が決定されている
- [ ] 依存する外部サービスの障害に対する Fallback / Circuit Breaker 方針が定義されている
