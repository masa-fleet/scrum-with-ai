...
- [ ] ログ収集方針（対象レベル・構造化ログ形式・保持期間）が定義されている
- [ ] メトリクス収集対象（例: CPU・メモリ・リクエスト数・エラー率）が定義されている
- [ ] 分散トレーシング採用有無と使用ツール（例: OpenTelemetry）が決定されている
- [ ] アラート条件と通知チャンネル（例: エラー率 > 1% → Teams / PagerDuty）が定義されている
- [ ] デプロイ戦略（例: Blue/Green、カナリア、ローリングアップデート）が決定されている
- [ ] デプロイ頻度の目標（例: 1 スプリントに 1 回本番リリース）が定義されている
- [ ] オンコール体制とエスカレーションフローが定義されている
