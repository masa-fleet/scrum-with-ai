...
- [ ] 認証・認可方式（例: OAuth 2.0 / OIDC、RBAC/ABAC）が決定されている
- [ ] 通信の暗号化要件（例: TLS 1.2+）が決定されている
- [ ] 保存データの暗号化要件（例: AES-256 at rest）が決定されている
- [ ] シークレット管理方式（例: Azure Key Vault、GitHub Secrets）が決定されている
- [ ] 準拠すべき法令・規制・業界標準（例: GDPR、PCI DSS、ISO 27001）が列挙されている
- [ ] 脆弱性管理方針（例: Dependabot、CodeQL、ペネトレーションテスト頻度）が決定されている
- [ ] セキュリティインシデント対応フロー（検知 → 通知 → 封じ込め → 回復）が定義されている
